...
export class InscripcionService {
  ...
  crear(inscripcion: Inscripcion): Observable<Inscripcion> {
    return this.http.post<Inscripcion>(`http://localhost:9898/api/inscripcion/`, inscripcion).pipe(
  ...
  //Editar Inscripcion
  editar(inscripcion: Inscripcion, idInscripcion: number): Observable<Inscripcion> {
    return this.http.put<Inscripcion>(`http://localhost:9898/api/inscripcion/actualizarInscripcion/${idInscripcion}`, inscripcion).pipe(
        map((response: any) => response.inscripcion as Inscripcion),
        catchError((e) => {
          if (e.status == 400) {
            return throwError(e);
